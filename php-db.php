...
class Update {
        ...
        foreach ($UPDATE_LOCAL->workdir as $item) 
        {
            ...
            foreach ($UPDATE_REMOTE->workdir as $remoteItem) 
            {
                ...
                if ($item->path === $remoteItem->path) 
                {
                    ...
                    $lokalChanged = $this->GetLocalDateTimeFromString($item->changed);
                    $remoteChanged = $this->GetLocalDateTimeFromString($remoteItem->changed);

                    if ($lokalChanged < $remoteChanged) 
                    {

                        $this->FetchUpdate($item->path);
    ...
    private function FetchUpdate($path) {
    ...
    private function GetLocalDateTimeFromString($datetimeString) {
